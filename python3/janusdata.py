        ...
        while True:
            ...
            for incoming_name in incoming_names:
                ...
                if not copy_err and os.path.isfile(path=img_orig_url):
                    img_size = int(os.stat(img_orig_url).st_size)
                    log = 'Copied image {0} with file size {1}'.\
                        format(img_orig_url, img_size)
                    logger.info(log)
                    print(log)

                    if img_size == 0:
                        os.remove(img_orig_url)
                        log = 'Removed image {0} with 0 KB file size'. \
                            format(img_orig_url)
                        logger.warning(log)
                        print(log)
                        ...
                        if (img_orig.shape[0] == 2464) and (img_orig.shape[1] == 3280):
                            ...
                            pass
                        elif (img_orig.shape[0] == 1000) and (img_orig.shape[1] == 1000):
                            pass
                        else:
                            continue
